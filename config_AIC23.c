...
/***************************************************/
static unsigned short myAIC23_registers[10] = { \										// READ THE HANDBOOK, 9 bit for each register
...
void Config_DSK6713_AIC23(void)
{
    ...
    MCBSP_config(hMcbsp0, xxxxx);		// WHICH DATA SCRUCT
    MCBSP_start(hMcbsp0, xxxxxx, 220);	// START FOR WHAT
    ...
    set_aic23_register(hMcbsp0,15,0);		// HAT 11 register + 12(welches 15 ist, und das reset) Steht in der Doku.
...
static void set_aic23_register(MCBSP_Handle hMcbsp,unsigned short regnum, unsigned short regval)	// 16 bit data word to the configuration, im handbook lesen, die lower 9 sind der Kontent und die oberen 7 sind der rest der register! Nummer der register, combine them in on value
{
    ...
    regval &= 0x1ff;		// untere rausgezogen SHIFT IT UP AND ORDER IT. DANN HAT MAN DAS WORD DAS GESCHREIBEN WIRD
    						// START UP PHASE ALSO ALLES EASY OB ES SLEEP; POLLT, but here, we are polling.
    /* warten */
    while (?????MCBSP_xrdy(xxxx));	// 1 when ready, 0 when not ready
