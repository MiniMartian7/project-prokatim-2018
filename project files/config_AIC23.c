...
void Config_DSK6713_AIC23(void)
{
	...
	hMcbsp0 = MCBSP_open(MCBSP_xxx, xxxxxx); //think which mcbsp is connected to parametrization
    ...
    set_aic23_register(hMcbsp0,15,0); // when you start parametrizing, you first should write reg15, then pwr down reg6
...
static void set_aic23_register(MCBSP_Handle hMcbsp,unsigned short regnum, unsigned short regval)
{
    ...
    /* zur Sicherheit maskieren auf 9 Bit */ //the lower 9 bit are the contents of register, the upper 7 are the address
    regval &= 0x1ff; //we mask the value of the lower 9 bit, take reg number, shift it up, and OR it with the value in one word
    
    /* warten */
    while (?????MCBSP_xrdy(xxxx)); //waiting mcbsp is ready, gives 1 when it's ready
    	// stay in the loop until it's not ready
